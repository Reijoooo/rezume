...
class Search_PG:
    ...
    g_url_list_unique = []
    def __init__(self):
        self.dblink = psycopg2.connect(dbname="spider", host="127.0.0.1", user="blinomes", password="",
                                       port="5433")
    ...
    def getWordsIds(self, queryString):
        ...
        cursor.execute(""" SELECT a.rowid, a.word from word a WHERE a.word IN %s """, (tuple(queryWordsList),))
        res = cursor.fetchall()
        if len(res) == 0:
        ...
        cursor.execute(""" select sum(res_final.rang) 
                             from (select res.*, res.score/res.count_url rang
                                   from (select a.*, b.score score, (select count(*)
                                                                       from link_between_url c 
                                                                      where c.fk_from_url_id = a.fk_from_url_id) count_url
                                           from link_between_url a, pagerank b
                                          where a.fk_to_url_id = %s
    ...
    def getUrlNameByUrlId (self, id):
        cursor = self.dblink.cursor()
        cursor.execute(""" select a.url from url a where a.rowid = %s""", [id])
        res = cursor.fetchall()
        if len(res) == 0:
            return None
        else:
    ...
    def calculatePageRank(self, iterations=3):
        ...
        for i in range(iterations):
            ...
            for j in range(len(url_list)):
                ...
                # обновляем таблицу
                cursor.execute(""" update pagerank set score = %s where urlid = %s""", [rang, url_id])

        # вычисляем ранг по нужным нам страницам
        cursor.execute(""" select urlid, score from pagerank where urlid in %s""", (tuple(self.g_url_list_unique),))
    ...
    def writeMarchRowsToCsvFile(self, rows):
        ...
        for row_local in rows:
            ...
            data.append(row)
        # запись в файл
        with open('match_rows.csv', 'w') as f:
            writer = csv.writer(f)
    ...
    def getMatchRows(self, queryString):
        ...
        for i in range(count_word):
            if i == 0:
                ...
                sqlpart_Join = sqlpart_Join + f" from word_location w{i} "
                ...
                sqlpart_Join = sqlpart_Join + f" inner join word_location w{i} on w{i-1}.fk_URLid = w{i}.fk_URLid"
    ...
    def getAllTextByUrlId(self, url_id):
        ...
        cursor.execute("""  select a.location, b.word 
                               from word_location a, word b 
                              where a.fk_wordid = b.rowid 
                                and a.fk_urlid = %s""", [url_id])
        alltext = ""
    ...
    def getSortedList(self, queryString):
        ...
        for i in range(1,4):
            if i>=len(result_table):
                break
            self.createMarkedHtmlFile(f"html_out{i}.html", self.getAllTextByUrlId(result_table[i][0]), queryString)
