...
class OLD_Crawler:
    ...
    def __init__(self, dbFileName):
        ...
        self.urlCountList = []  # Список для хранения количества URL
        self.wordCountList = []  # Список для хранения количества слов
    ...
    def isIndexed(self, url_id):
        ...
        cursor.execute("SELECT 1 FROM wordLocation WHERE fk_URLid = ?", (str(url_id),))
        res = cursor.fetchone()
        cursor.close()
    ...
    def addLinkRef(self, urlFrom, urlTo, linkText):
        ...
        # Связываем ссылки
        cursor.execute("INSERT INTO linkBetweenURL (fk_from_url_id, fk_to_url_id) VALUES (?, ?)",
                       (str(urlFrom), str(urlTo)))
        link_ref_id = cursor.lastrowid
    ...
    def addUrlToTableUrl(self, url):
        cursor = self.dblink.cursor()
        cursor.execute("SELECT * FROM URLList WHERE url = ?", (url,))
        res = cursor.fetchall()
        if len(res) == 0:
            cursor.execute("INSERT INTO URLList (url) VALUES (?)", (url,))
            self.dblink.commit()  # Сохраняем изменения
    ...
    def addWorldToWorldLocation(self, fk_word_id, fk_url, location):
        ...
        cursor.execute("INSERT INTO wordLocation (fk_wordid, fk_URLid, location) VALUES (?, ?, ?)",
                       (str(fk_word_id), str(fk_url), str(location)))
        self.dblink.commit()  # Важно подтвердить изменения в базе данных
        cursor.close()
    ...
    def addWordToTableWord(self, word, isFiltres=0):
        ...
        if word.istitle() == True:
            ...
            cursor = self.dblink.cursor()
            cursor.execute("SELECT * FROM wordList WHERE word = lower(?)", (word,))
            res = cursor.fetchall()
